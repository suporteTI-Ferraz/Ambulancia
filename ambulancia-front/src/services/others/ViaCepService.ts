...
export const buscaCepPaciente = async (cep: string) => {
    try {
  ...
  export const buscaCepHosp = async (cep: string) => {
    try {
      // Remove caracteres não numéricos do CEP
      const cepLimpo = cep.replace(/\D/g, "");
      // Verifica se o CEP possui 8 dígitos
      ...
      return {
        ruaHosp: response.data.logradouro || "",
        bairroHosp: response.data.bairro || "",
        cidadeHosp: response.data.localidade || "",
        estadoHosp: response.data.uf || "",
      };
    } catch (error: any) {
      console.error("Erro ao buscar o CEP:", error.message || error);
      throw new Error(error.message || "Erro ao buscar o CEP.");
